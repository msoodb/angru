...
SecurityRolesModel::SecurityRolesModel(){}
SecurityRolesModel::~SecurityRolesModel(){}

pqxx::result SecurityRolesModel::GetSecurityRoless(int page, std::string query){
...
int SecurityRolesModel::GetSecurityRolessCount(std::string query){
	pqxx::connection C(angru::wrapper::Postgresql::connection_string());
	try {
		if (C.is_open()) {
			 LOG_INFO << "Opened database successfully: " << C.dbname();
...
boost::property_tree::ptree SecurityRolesModel::GetSecurityRolessJson(int page, std::string query){
	pqxx::result R = GetSecurityRoless(page, query);
	int result_count = GetSecurityRolessCount(query);
...
pqxx::result SecurityRolesModel::GetSecurityRoles(std::string id){
	pqxx::connection C(angru::wrapper::Postgresql::connection_string());
	try {
		if (C.is_open()) {
			 LOG_INFO << "Opened database successfully: " << C.dbname();
...
boost::property_tree::ptree SecurityRolesModel::GetSecurityRolesJson(std::string id){
	pqxx::result R = GetSecurityRoles(id);
	boost::property_tree::ptree security_role_node;
...
std::string SecurityRolesModel::AddSecurityRoles(
													std::string	name, 
													std::string	title, 
...
void SecurityRolesModel::UpdateSecurityRoles( 
													std::string	id,
													std::string	name,
													std::string	title,
...
void SecurityRolesModel::DeleteSecurityRoles(std::string id){
	pqxx::connection C(angru::wrapper::Postgresql::connection_string());
	try {
		if (C.is_open()) {
			 LOG_INFO << "Opened database successfully: " << C.dbname();
