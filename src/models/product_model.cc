...
pqxx::result ProductModel::GetProducts(int page, std::string query){
	...
	std::string complete_query = "SELECT \
									      				...
									      				taxable, \
									      				description, \
																(select count(product_documents.id) from product_documents where product_id = products.id and deleted_at is NULL) as attachments \
...
std::string ProductModel::AddProduct(
	...
	LOG_INFO << "Connected to database: " << C.dbname();
	pqxx::work W(C);
	C.prepare("insert", "INSERT INTO product ( \
													id, \
													title, \
													price, \
													created_at, \
