...
public:
	TagsContentModel();
	~TagsContentModel();
	static pqxx::result GetTagsContents(int page=1, int limit=LIMIT_COUNT, std::string query="", std::string order="");
	static int GetTagsContentsCount(std::string query="");
	static boost::property_tree::ptree GetTagsContentsJson(int page=1, int limit=LIMIT_COUNT, std::string query="", std::string order="");
